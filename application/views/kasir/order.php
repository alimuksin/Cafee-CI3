			...
			<table id="basic-datatables" class="table table-striped table-sm table-head-bg-primary"  width="100%">
				...
				<tbody>
					...
					<tr id="<?= $key->id_checkout; ?>">
						...
						<td>
                            <?= transaksi($this->global->kasir_transaksi($key->id_checkout)->status_transaksi) ?>
                        </td>
						<td>
							<div class="form-button-action">
								<a  title="" class="btn btn-link btn-primary btn-lg" href="<?=base_url('kasir/orderDetail?orderId='.$key->id_checkout); ?>">
									<i class="fa fa-info"></i>
								</a>
								<a href="<?=base_url('kasir/hapusorder/'.$key->id_checkout); ?>" data-toggle="tooltip" title="" class="btn btn-link btn-danger" data-original-title="Remove">
